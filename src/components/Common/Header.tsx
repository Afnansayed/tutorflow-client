      ...
      >
        <div className="container-max  flex items-center justify-between gap-4">
              ...
              <div key={link.path}>
                {"submenu" in link ? (
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="w-48 p-2 rounded-2xl shadow-2xl border-slate-100">
                      {link.submenu.map((sub: NavSubItem) => (
                        <DropdownMenuItem key={sub.path} asChild>
                          <Link href={sub.path} className="w-full cursor-pointer rounded-xl font-medium">
                            {sub.label}
                          </Link>
                        </DropdownMenuItem>
                      ))}
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      pathname === link.path ? "text-primary bg-primary/5" : "text-slate-600 hover:text-primary"
                    )}
                  >
                    {link.label}
                  </Link>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <Button variant="ghost" className="hidden md:flex rounded-full font-bold text-slate-600">
              Log In
            </Button>
            ...
            <div className="lg:hidden">
              <Sheet>
                ...
                <SheetContent side="right" className="w-[300px] border-l border-slate-100 bg-white/95 backdrop-blur-xl p-0">
                      ...
                      <div key={link.path}>
                        {"submenu" in link ? (
                          <div className="space-y-2">
                            <div className="px-4 py-3 text-sm font-bold text-slate-400 uppercase tracking-widest">{link.label}</div>
                            {link.submenu.map((sub) => (
                              <Link key={sub.path} href={sub.path} className="flex items-center gap-3 p-4 rounded-2xl bg-slate-50 font-semibold text-slate-700">
                                {sub.label}
                              </Link>
                            ))}
                          </div>
                        ) : (
                          <Link href={link.path} className="flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 font-bold text-slate-700 transition-colors">
                            {link.label}
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-6 bg-slate-50 border-t flex flex-col gap-3">
                    <Button variant="outline" className="w-full h-12 rounded-2xl font-bold border-2">Log In</Button>
                    <Button className="w-full h-12 rounded-2xl font-bold bg-primary">Get Started</Button>
                  </div>
                </SheetContent>
              </Sheet>
            </div>
